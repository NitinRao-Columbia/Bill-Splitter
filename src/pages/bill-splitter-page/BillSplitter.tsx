...
        setEmail('');
        setError('');
        setConfirmationMessage(`Participant ${email} has been successfully added to the bill!`);
...
    try {
      const response = await axios.post(
        `http://3.137.160.197:8000/bills/${billId}/items/${itemId}/pay`,
        {
          user_id: '9f8ae3f2e2c54eeb995c5531c770498a',
        }
      );

      if (response.status === 200) {
        // Mark the item as paid in local state (only the clicked item)
        setPaidStatus((prevStatus) => ({
          ...prevStatus,
          [itemId]: true, // Set the clicked item's status to 'true'
        }));
...
          />
          <button onClick={handleAddParticipant} className="btn" style={{ marginLeft: '0.5rem' }}>
            Add Participant
          </button>
        </div>
